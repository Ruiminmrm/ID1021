        ...
        while(isTrue) { //kommer vara sant hela tiden tills vi avslutar för att slippa läsa om filen
            ...
            switch (inp) {
                case 1:
                    ...
                    BreadthFirstPaths bfp = new BreadthFirstPaths(Graf, sp); //skapa en ny dfp
                    Iterator iterator = graf.keys().iterator();
                    String at = graf.min();

                    if(bfp.hasPathTo(slp)) { //finns det en väg sådan att vi kan gå från stat 1 till stat 2?
                        System.out.println(start + " till " + slut + " går denna väg: ");
                        for (int a : bfp.pathTo(slp)) { //Iterera över vägarna
